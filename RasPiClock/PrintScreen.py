...
def Twitch(conf, TextPAPIRUS): #Fonction Twitch
	...
	try:
		if DataSt1["data"][0]["type"] == "live":
			ReponseTwitchGameID = requests.get("https://api.twitch.tv/helix/games?id=" + DataSt1["data"][0]["game_id"], headers={"Client-ID": conf["TWITCH"]["TwitchAPI"]})
	...
	try:
		if DataSt2["data"][0]["type"] == "live":
			ReponseTwitchGameID = requests.get("https://api.twitch.tv/helix/games?id=" + DataSt2["data"][0]["game_id"], headers={"Client-ID": conf["TWITCH"]["TwitchAPI"]})
			GameID = json.loads(ReponseTwitchGameID.text)
